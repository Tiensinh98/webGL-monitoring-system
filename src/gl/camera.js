...
export default class Camera {
	...
	zoom(scale, x, y) {
		...
		const sx = width / 2 - x;
		const sy = height / 2 - y;
      	var t = Translation.fromTranslation(vec3.fromValues(sx, sy, 0));
		var s = Transforation.fromScaling(scale);
		var tInv = Translation.fromTranslation(vec3.fromValues((-sx, -sy, 0));
